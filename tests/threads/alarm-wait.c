...
sleeper(void *t_)
{
  ...
  for (i = 1; i <= test->iterations; i++)
  {
    ...
    // -> (sleep할 시간 - 현재 시간)만큼 sleep할 것이다.
    lock_acquire(&test->output_lock);
    printf("tick : %d, id : %d\n", timer_ticks(), t->id);
    // 해당 test의 대상이 thread임
    // 정상적인 (우리가 원하는) 출력을 위해서는 thread의 원자성을 보호해야함.
    // -> 해당 thread의 원자성을 보호하기 위해서는 lock을 하고 realease를 해야함. 이를 위해 output_lock을 만듦.
